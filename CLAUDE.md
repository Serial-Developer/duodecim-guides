...
- **Le serveur de preview est `node scripts/serve.mjs`** (branché dans `.claude/launch.json` et `npm run serve`). `python -m http.server` réinitialise la connexion sur `build-data.js` (355 ko, `ERR_CONNECTION_RESET` en boucle) : le créateur démarre alors sans données et tout semble cassé. `npx serve` n'est pas une option ici — le paquet n'est pas installé et `npx` est un `.ps1` sous Windows.
- **Le lien de partage est compressé** (`deflate-raw` + base64url, préfixe `z`) : 1595 → 716 caractères sur un build complet. L'encodage et le décodage sont donc **asynchrones**. Sans préfixe `z`, le paramètre est lu comme l'ancienne base64 brute — les liens déjà partagés doivent continuer de s'ouvrir, ne pas retirer cette branche.
