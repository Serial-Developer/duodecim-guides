...
// Une entité WebSite par langue : les deux versions ont leur propre URL d'entrée,
// leur propre nom et leur propre `inLanguage`. Un identifiant unique partagé les
// écraserait l'une l'autre et déclarerait le site dans une seule langue.
const websiteId = (locale) => `${SITE_URL}/#website-${locale}`;
const AUTHOR_ID = `${SITE_URL}/#author`;

// Licence des textes originaux (cf. LICENSE et NOTICE.md)
...
export function ldWebSite({ description, locale, name, path, datePublished, dateModified }) {
...
export function ldArticle({
  type = 'TechArticle', headline, description, path, locale, image, imageAlt,
...
export function ldWebApplication({ name, description, path, locale, browserRequirements, image, imageAlt, datePublished, dateModified }) {
  const url = absUrl(path);
  const out = {
    '@context': 'https://schema.org',
    '@type': 'WebApplication',
